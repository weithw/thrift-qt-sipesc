...
public MExtension
  ...
  public:
    void showMessage2(const QString& message) const;
  };
...
public MExtensionObject
  ...
  public:
    virtual void showMessage2(const QString& message) const=0;
  private:
...
inline QUuid MessageReceiverInterface::getUuid()
  {
  QLatin1String uuid("{c7ac15b5-31f7-40c1-b334-e60fc4657a5a}");
  return QUuid(uuid);
  }
...
inline void MessageReceiver::showMessage2(const QString& message) const
  {
  MessageReceiverInterface* _ptr;
  _ptr=getPrivateData<MessageReceiverInterface*>();
  if(_ptr==0) return;
  _ptr->showMessage2(message);
  }
